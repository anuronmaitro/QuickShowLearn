...
import { Menu, MenuIcon, SearchIcon, TicketPlus, XIcon } from 'lucide-react'
...
const Navbar = () => {
  /*
      ...
      initialValue → the starting value (false here)
  */
  ...
  return (
    ...
    justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ?'max-md:w-full':'max-md:w-0'} `}>
        {/* croos icon for mobile */}
         <XIcon className='md:hidden absolute top-6, right-6 w-8 h-8 cursor-pointer' onClick={()=>setIsOpen(!isOpen)}/>
      ...
      hover:bg-primary-dull transition rounded-full font-medium'>Login</button>) 
      :
      ( <UserButton>
      ...
      {/* menu icon */}
      {/* will be displaed on small screen only */}
